...
public class Resource {
    ...
    public void checkIncrement(long delta) {
        if (current + delta > max) {
            throw new com.prateek.sandbox.runtime.ResourceLimitException(
    ...
    public void increment(int[] counts, int size) {
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            if (counts[i] > Long.MAX_VALUE / product) {
                throw new com.prateek.sandbox.runtime.ResourceLimitException("Trying to allocate more than 2^63 bytes of memory");
            }
